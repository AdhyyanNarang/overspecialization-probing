"""
Plotting utilities for Amazon reviews MSGD experiments.
...
Adapted from utils_plotting.py (Census experiments).
"""
...
def _amazon_model_colors(n_models):
    """
    Return model colors for Amazon plots.

    We swap learner-2 and learner-3 colors (green <-> purple) so the
    probing learner color stays consistent with other dataset figures.
    """
    rgb_palette = [
...
def plot_individual_model_accuracies(results_dict, args, title=None, only_p0=False, show_title=True, show_legend=True, save_file_name=None, baseline_acc=None):
    ...
    y_min, y_max = 0.4, 0.85
    if baseline_acc is not None:
        y_min = min(y_min, baseline_acc - 0.02)
        y_max = max(y_max, baseline_acc + 0.02)

    colors = _amazon_model_colors(n_models)
...
def plot_individual_model_accuracies_with_kappa(results_dict, title=None):
    ...
    plt.savefig("amazon_individual_model_accuracies_by_kappa.png", dpi=300, bbox_inches="tight")
    plt.show()


def plot_assignment_fraction(
    results_dict,
    args,
    title=None,
    max_iter=None,
    *,
    ...
    Args:
        ...
        style: 'lines' (default) or 'stacked'
        smooth_window: Rolling window size for smoothing (1 disables)
        max_points: Downsample to at most this many points (None disables)
        show_stderr: Whether to draw stderr bands (lines only)
    """
    plt.style.use('seaborn-v0_8-whitegrid')
    mpl.rcParams.update({
        'axes.titlesize': 16,
    ...
    def smooth_series(array, window):
        if window is None or window <= 1:
            return array
        kernel = np.ones(window, dtype=array.dtype) / window
        return np.vstack([np.convolve(array[:, k], kernel, mode='same') for k in range(array.shape[1])]).T
    ...
    for j, p in enumerate(p_values):
        ...
        mean_frac = smooth_series(mean_frac, smooth_window)
        stderr_frac = smooth_series(stderr_frac, smooth_window)
        if max_points and len(iters) > max_points:
            stride = int(np.ceil(len(iters) / max_points))
            iters = iters[::stride]
            mean_frac = mean_frac[::stride]
            stderr_frac = stderr_frac[::stride]
        if style == 'stacked':
            ax.stackplot(iters, mean_frac.T, colors=colors, labels=[f"Learner {k}" for k in range(n_models)])
            ax.set_ylim(0, 1.0)
        else:
            for k in range(n_models):
                ax.plot(iters, mean_frac[:, k], color=colors[k], linewidth=1.8, label=f"Learner {k}")
                if show_stderr:
                    ax.fill_between(
                        iters,
                        mean_frac[:, k] - stderr_frac[:, k],
                        mean_frac[:, k] + stderr_frac[:, k],
                        color=colors[k],
                        alpha=0.12
                    )
    ...
    plt.tight_layout()
    plt.savefig("amazon_model_assignment_fraction.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_user_assignment_map(results_dict, X_train, *, p_value, kappa=0.0, seed=None, title=None, method='pca'):
    ...
    if title:
        fig.suptitle(title, fontsize=16)
    plt.tight_layout()
    plt.savefig(f"amazon_user_assignment_map_p{p_value}.png", dpi=300, bbox_inches='tight')
    plt.show()
