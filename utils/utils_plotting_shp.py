"""
Plotting utilities for Stanford SHP MSGD experiments.
...
Adapted from utils_plotting_amazon.py.
"""
...
def plot_individual_model_accuracies(results_dict, args, title=None, only_p0=False, show_title=True, show_legend=True, save_file_name=None):
    """
    ...
    Args:
        ...
        save_file_name (str): Base name for the saved file (without extension). If None, defaults to 'shp_msgd_results'.
    """
    ...
    base_name = save_file_name if save_file_name else "shp_msgd_results"
...
def plot_final_accuracy_vs_p(results_dict, args, title=None, save_file_name=None):
    """
    ...
    Args:
        ...
        title: Optional title for the figure
        save_file_name: Base name for the saved file (without extension). If None, defaults to 'shp_final_accuracy_vs_p'.
    """
    plt.style.use('seaborn-v0_8-whitegrid')
    mpl.rcParams.update({
        'font.family': 'sans-serif',
        ...
        if learner_idx in probing_set:
            ...
            for kappa_idx, kappa in enumerate(kappa_values):
                ...
                if p_vals_for_plot:
                    ...
                    label = f"Learner {learner_idx} (κ={kappa})" if kappa > 0 else f"Learner {learner_idx} (κ={kappa})"
    ...
    ncol_legend = max(2, (len(handles) + 1) // 2)
    legend = ax.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, -0.1),
                      ncol=ncol_legend, fontsize=12, frameon=True,
                      fancybox=True, shadow=True, borderpad=1)
    legend.get_frame().set_facecolor('white')
    legend.get_frame().set_alpha(0.95)
    legend.get_frame().set_edgecolor('gray')
    ...
    # Use provided name or default
    base_name = save_file_name if save_file_name else "shp_final_accuracy_vs_p"
...
def plot_individual_model_accuracies_with_kappa(results_dict, title=None):
    ...
    plt.savefig("shp_individual_model_accuracies_by_kappa.png", dpi=300, bbox_inches="tight")
    plt.show()
...
def plot_assignment_fraction(results_dict, args, title=None, max_iter=None):
    ...
    plt.tight_layout()
    plt.savefig("shp_model_assignment_fraction.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_user_assignment_map(results_dict, X_train, *, p_value, kappa=0.0, seed=None, title=None, method='pca'):
    ...
    if title:
        fig.suptitle(title, fontsize=16)
    plt.tight_layout()
    plt.savefig(f"shp_user_assignment_map_p{p_value}.png", dpi=300, bbox_inches='tight')
    plt.show()
